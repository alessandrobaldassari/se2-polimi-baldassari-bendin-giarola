...
%			\vspace*{-0.35cm}
	...
	This algorithm depicts what happens when the system assigns the taxi to a specific zone according to the taxi's current position obtained through the GPS sensor of the driver's phone. When the taxi is assigned it enters the queue at the bottom.\\
	\lstinputlisting[language=C]{Pseudocode/QueueManager.c}
	\begin{minipage}{\linewidth}
%			\vspace*{-0.35cm}
		\makebox[\linewidth]{
	...
	This algorithm depicts what happens when the driver switches his/her own availability. If s/he turns it \textit{on} then the system will proceed with the zone assignment according to the position, while if the driver switches the availability \textit{off} the system will simply remove him/her from the current queue.\\
