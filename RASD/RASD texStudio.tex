...
%	\addtocounter{page}{-1}
	...
	\subsection{Assumptions and dependencies}
		\begin{itemize}
			...
			\item Passengers always have enough money to pay the taxi fare and the payment data provided are valid. (\textcolor{red}{even because there is no way to predict where the passenger will go when s/he calls for an immediate pickup, the nthere is no way to predict the final fare!})
...
%	\pagebreak
	...
	\paragraph{Your work} The taxi driver's home page: s/he can accept or reject the requests forwarded by the system and set her/his own ``availability''. \textcolor{red}{There is also the possibility to inform the system about an ``emergency situation'' and the impossibility to carry out the scheduled jobs. Does the system also offer a sat nav function to the driver?}
	\begin{center}
	...
	\begin{itemize}
		\item {[}R1{]} The mobile phone must have the GPS. \textcolor{red}{(HW requirement or assumption? maybe given by the company...)}
	...
	\begin{itemize}
		\item {[}R1{]} The cancellation of a ride could imply a fee automatically collected by the system using the user's preferred payment method.
		\item {[}R2{]}
		\item {[}D1{]}
	\end{itemize}
	...
	\begin{itemize}
		\item {[}R1{]} The user can check his/her own reservations and possibly edit or delete a reservation for a taxi. (???maybe only with a constraint eg. 30 minutes in advance, there is however a booking fee??)
		\item {[}R2{]}
		\item {[}D1{]}
	\end{itemize}
	...
	\begin{itemize}
		\item {[}R1{]} This is only valid for reservations booked in advance.
		\item {[}R2{]}
		\item {[}D1{]}
	\end{itemize}
	...
	\begin{itemize}
		\item {[}R1{]} The user can keep track of the favorite routes, favorite payment method? Link credit card? Maybe to pay the fee in case the user decides to delete a reservation?
